...
exports.run = (client, message, args) => {
  ...
  if (message.mentions.users.size < 1) return message.reply('** Kimi Banlayacam Söyle Bana? **').catch(console.error);
  ...
  const embed = new Discord.RichEmbed()
    ...
    .setTimestamp()
    .addField('İş:', 'Ban')
    .addField('BanHammer Atılan Kişi:', `${user.username}#${user.discriminator} (${user.id})`)
    .addField('BanHammerı Atan Kişi:', `${message.author.username}#${message.author.discriminator}`)
